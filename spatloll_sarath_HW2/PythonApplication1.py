...
################################################################################
samplelr=[1e-7,0.8e-7,0.6e-7,0.4e-7]
samplereg=[4.6e4,4.8e4,5e4,5.2e4,5.4e4,5.6e4]

for i in samplelr:
    for j in samplereg:
        classifier =Svm(xTrain.shape[1],numClasses)
        classifier.train(xTrain, yTrain, lr=i, reg=j, iter=1500, verbose=False)
        tempacc=classifier.calAccuracy(xVal, yVal)
        if tempacc>bestAcc:
            bestAcc=tempacc
            bestParameters=[i,j]
...
# - Store the best accuracy in bestAcc                                         #
################################################################################
samplelr=[le-7,0.8e-7,0.6e-7,0.4e-7]
samplereg=[4.6e4,4.8e4,5e4,5.2e4,5.4e4,5.6e4]
