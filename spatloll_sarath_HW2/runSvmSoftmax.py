...
learningRates = [1e-7, 2e-7, 2.5e-7, 1.9e-7, 1.5e-7, 9e-6,4e-7, 1e-6, 2e-6]
regularization = [1e4, 0.5e4, 0.4e4, 2e4, 1e5, 3e2, 2e3]

iters = 1500
for lr in learningRates:
    for rs in regularization:
        classifier = Svm(xTrain.shape[1], numClasses)
        classifier.train(xTrain,yTrain, lr, rs, iters, verbose=True)
        accTrain = classifier.calAccuracy(xTrain, yTrain)
        accVal = classifier.calAccuracy(xVal, yVal)
...
learningRates = [1e-7, 2e-7, 2.5e-6, 4e-7, 6e-7, 1e-6]
regularization = [1e4, 0.5e4, 2e4, 3e2, 2e3, 1e1]
